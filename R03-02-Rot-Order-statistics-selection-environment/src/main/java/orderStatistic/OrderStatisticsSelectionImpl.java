...
public class OrderStatisticsSelectionImpl<T extends Comparable<T>> implements OrderStatistics<T> {
	...
	public T getOrderStatistics(T[] array, int k) {
		if (!(array.length == 0 || array == null)) {
			T menorKordem;
			menorKordem = array[0];
			menorKordem = getStatistics(array,0, array.length, k) ;
			return menorKordem;
		}
		return null;
	...
	private T getStatistics(T[] array, int leftIndex, int rightIndex, int k) {
		int cont = 1;
		int minIndex = 0;
		T retorno= array[minIndex];
			for (int j = minIndex + 1; j <= array.length; j++) {
				if (array[j].compareTo(array[minIndex]) < 0) {
					minIndex = j;
					cont++;
					retorno = array[minIndex];
				}
			}
			if(!(cont == k)) getStatistics(array, minIndex+ 1, array.length,k);
			return retorno;
